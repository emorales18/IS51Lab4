...
question = "What is the capital of California". The answer of "Sacramento".

We first set max_tries = 3. Then we create a loop to iterate three times. For each iteration,
we as the user for the answer (user input). Then based on the answer the user gives, we check 
to see if the user input matches the answer. If som print "Correct!", then terminate the loop
...
ask
    ...
    loop three times
        ...
        ask user input()
        check to see if user inout is equal to answer
            if so, print "Correct" then exit loop
    if not correct
        print to the user "You have used up allotment of guesses."
